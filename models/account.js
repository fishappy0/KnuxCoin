...
module.exports.verifyAccount = async function (uname, passphrase) {
    ...
    if(returnedQuery == null) {
        ...
        const isSamePassword = await dbg(bcrypt.compare(passphrase, returnedQuery['password']));
        if (isSamePassword)  {
...
module.exports.createAccount = async function (phone) {
    ...
    let account_id = current_time.getFullYear().toString().slice(2)+ "U" +(Math.round(Math.random() * 10000000)).toString();
    ...
    let hashed_password = await bcrypt.hash(login_password,
